...
predadores-own [
  is-predator?         ;; A mesma variável para predadores, mas agora para a raça 'predadores'
]
...
to setup
  ...
  create-predadores 3 [
    ...
    set is-predator? true ;; Garantir que o predador tenha a variável "is-predator?" definida como true
  ]
...
to hunt
  let target one-of formigas with [distance myself < 3]  ;; Seleciona uma formiga dentro de um raio de 3 unidades
  if target != nobody [
    face target
    fd 0.5  ;; Move-se em direção à formiga aos poucos
    if distance target < 1 [  ;; Quando estiver muito perto, come a formiga
      ask target [
        die  ;; Remove a formiga
      ]
